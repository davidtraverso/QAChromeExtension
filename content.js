...
if (window.location.href.includes(qualPage)) {
  ...
  function extractNotes(str) {
    ...
    var note = '';
    budgetField[0].value ? (note = budgetField[0].value) : (note = '** Empty Note **');
    return note;
  }
  ...
  var budgetDivStyles =
    'margin:30px 0 0 330px; padding:0 8px 0 8px; background:rgba(253, 128, 11, 0.3); text-align:left; visibility:hidden;';
  budgetDiv.setAttribute('style', budgetDivStyles);
  budgetDiv.innerHTML = `<p style="display:inline-block; margin:4px 0 0 0;">Budget: ${budgetNote}</p>`;
...
if (window.location.href.includes(queuePage)) {
  ...
  setTimeout(function() {
    ...
    [...deletedQueueData].map(e => {
      e.innerText.search(regex) !== -1 && e.innerText.search(deletedDead) == -1 ? matches.push(e) : nonMatches.push(e);
    });
    // ** Add RUN or BREAK conditionality here **
